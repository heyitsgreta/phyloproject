...
# Extract top N unique hits by E-value from BLAST results
extract_top_hits <- function(df, n = 30) {
  df %>%
    distinct(sacc, .keep_all = TRUE) %>%
    slice_min(E_value, n = n)
}

# Save a phylo tree to PDF with standard formatting
save_tree_pdf <- function(tree, filename, title) {
  pdf(file.path(output_dir, filename), width = 12, height = 10)
  plot(tree, main = title, cex = 0.7)
  dev.off()
}
...
run_blast <- function(query, db, matrix = "BLOSUM62", outfile = NULL) {
  ...
  # Parse results using read.table with comment.char to skip # lines
  df <- read.table(outfile, sep = "\t", stringsAsFactors = FALSE,
  ...
  colnames(df) <- c("sacc", "Alignment_length", "Percent_identity",
                     "Gaps", "Score", "Bit_score", "E_value")
  df$E_value <- as.numeric(df$E_value)
  df$Percent_identity <- as.numeric(df$Percent_identity)

  # Sort by e-value ascending
  df <- df %>% arrange(E_value)
  return(df)
}
...
top30 <- extract_top_hits(blast_results)
...
top30 %>% select(sacc, Percent_identity, E_value, Bit_score) %>% print(row.names = FALSE)
cat("\n")
...
# Match top 30 accessions to sequence names using sapply pattern
seq_acc <- sub(" .*", "", names(all_seqs))
matched_idx <- sapply(top_acc, function(X) which(seq_acc == X))
matched_idx <- unlist(matched_idx)
...
rename_to_binomial <- function(seqs) {
  ...
  names(seqs) <- make.unique(binomials, sep = "_")
  return(seqs)
}
...
# Convert to AAStringSet using unmasked() for type conversion
msa_ss <- unmasked(msa_result)
msa_ss <- AAStringSet(msa_ss)

p_msa <- ggmsa(msa_ss, start = window_start, end = window_end,
               char_width = 0.5, seq_name = TRUE,
               color = "Chemistry_AA", font = "DroidSansMono") +
  geom_seqlogo() +
  geom_msaBar() +
...
save_tree_pdf(upgma_tree, "upgma_tree.pdf", "UPGMA Tree")
...
save_tree_pdf(nj_tree, "nj_tree.pdf", "Neighbor-Joining Tree")
...
save_tree_pdf(as.phylo(complete_hclust), "complete_linkage_tree.pdf", "Complete Linkage Tree")
save_tree_pdf(as.phylo(ward_hclust), "ward_tree.pdf", "Ward's Method Tree")
...
# Both trees are built from the same dist_mat, so labels are identical
shared_labels <- rownames(as.matrix(coph_upgma))
coph_upgma_mat <- as.matrix(coph_upgma)[shared_labels, shared_labels]
coph_nj_mat <- as.matrix(coph_nj)[shared_labels, shared_labels]

# Cophenetic correlation with original distances
orig_mat <- dist_matrix[shared_labels, shared_labels]
...
bootstrap_trees <- Filter(Negate(is.null), lapply(seq_len(n_boot), function(b) {
  ...
  if (!is.null(boot_dist) && all(is.finite(as.matrix(boot_dist)))) {
    tryCatch(nj(boot_dist), error = function(e) NULL)
  } else {
    NULL
  }
}))
